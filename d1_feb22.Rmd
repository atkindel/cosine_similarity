---
title: "Anisotropy and frequency bias in cosine similarity measures"
author: "Alexander T. Kindel^[PhD Candidate, Department of Sociology, Princeton University. Contact: akindel@princeton.edu]"
abstract: |
 |  Social scientists often use the cosine ratio to describe patterns of association in vector space models of social phenomena (e.g., word embeddings like word2vec or GloVe). It has recently been observed that some measures constructed in this manner exhibit a strong correlation with relative frequency. I show that the roots of this problem lie in the use of standard basis arithmetic to group and compare sets of cosine similarities, which do not imply a common basis. Means of sets of vectors selected arbitrarily from the underlying model imply a hyperelliptical subspace that is transformable to the unit ball; however, these hyperellipsoids are generally anisotropic (i.e., they have different shapes). When computed in the standard basis, numerical comparisons between these hyperelliptical measures implicitly upweight the contribution of comparisons involving relatively uneven group sizes; in these settings the relative "units" of the cosine similarity are larger. As a running example, I discuss frequency bias in the Word Embedding Association Test (WEAT), a widely-used measure of linguistic bias in text corpora that examines the difference in mean cosine similarity between sets of preselected words. I emphasize that the problem is attributable to methodology---particularly the arithmetic comparison of correlation coefficients---and not to statistical properties of word embedding models or inherent qualities of natural language.
 | 
 | **Keywords:** cosine similarity, word embeddings, algorithmic bias, frequency bias, measurement, correlation, arithmetic
date: |
  | 15 February 2022
  | 
  | 
