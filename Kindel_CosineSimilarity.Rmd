...
These two requirements can be combined formally in a general way. In the unidimensional setting, a well-known requirement for defining metrics with this property is the *minimality axiom*, which states that any object $x$ is most similar to itself [@tversky1977features]:  
...
\end{aligned}
$$
Consequently, the $k = 1$ case is special in the sense that the MCS score is equivalent to a nonlinear transformation of the general word association metric. Unfortunately, this is not generally true when $k > 1$. This fact creates substantial trouble for MCS-based measurement models.


### MCS is not a geometrically consistent choice of word association metric

We are now in a better position to understand the shortcomings of WEAT-like measurement models. The core issue is that the MCS score is not geometrically consistent, so it is an invalid association metric for multidimensional word association problems. Moreover, MCS is a biased estimator of the expected one-to-many alignment, so the estimator cannot be re-interpreted as an expectation over one of the keyword lists.


#### MCS is geometrically inconsistent for multidimensional word association problems.

Denote the subspaces of $\mathcal{R}(W)$ spanned by $X, Y$ as $W_X, W_Y$. Denote the power sets of $X, Y$ as $\text{Pow}(X), \text{Pow}(Y)$. Define the metric $d_{q}(W_X, W_Y)$ between the subspaces corresponding to every $q$-subset in $\text{Pow}(X), \text{Pow}(Y)$, where $q \in [1, k_z-1]$. As we increase $k$ to the ambient dimensionality of $W$, the intersection of the orthogonal complements of $W_X$ and $W_Y$ eventually vanishes; that is, regardless of the choice of words, the two subspaces will approach one-another until they coincide exactly once we hit the number of dimensions in the vector space. If $d_{q}(W_X, W_Y)$ is geometrically consistent with respect to $W$, then this must also be true everywhere on the lattice of similarity metrics defined by $\text{Pow}(X), \text{Pow}(Y)$ as $q$ increases to $k$. Thus $d_{q}(W_X, W_Y)$ must be nondecreasing as we increase $q$.
...
#### MCS is a biased estimator of the expected one-to-many word association.
...
In order to correctly average this quantity we need to take this integral with respect to spherically uniform measure

Arithmetic mean cosine similarity can be consistently interpreted as an estimator of the expected association between any two words in the input keyword lists only if one of the keyword lists contains only one word. If that condition holds, then the underlying set of angles can be interpreted as central angle parameters for the subgroup of orthogonal rotations that move the fixed vector onto the spans of the vectors in the other list [@moakher2002means]. However, if both keyword lists have more than one word, then the Euclidean arithmetic mean cosine is a biased estimator of the mean rotation. The bias is proportional to the difference between the number of vectors in the analysis and the square root of the determinant of the arithmetic sum of the corresponding rotation matrices:

TODO: Bias from Moakher 2002

Additionally, even if we do this, adding words to the inducing keyword lists still causes MCS to decrease as $k$ increases. This is because as $k$ approaches $p$, MCS converges in probability to the expected cosine between two vectors in $\mathbb{R}^p$ irrespective of the specific choice of input subspaces and basis dimensionality. This is the opposite of what we want the metric to do. As previously mentioned, if the word list is converging to a specific concept, we should gain *more information* about what we mean by adding to the word list, and not less. Consequently, even if the word association estimand were redefined to be the expected unidimensional word association between two sets of words, MCS is *also* a biased estimator for this quantity. $\mathbb{E}_{A,B}[\text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b))]$.



First note that $\mathbb{E}_{A,B}[\text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b))] = k^{-1}\text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b))$, yielding the following expression for the bias of the estimator:  
$$
\begin{aligned}
\text{Bias}(d^\text{MCS}(A, B)) &= \mathbb{E}\Big[\frac{1}{k}\sum_{a \in A}\sum_{b \in B}\sqrt{\text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b))}\Big] - \frac{1}{k}\text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b)) \\
&= \mathbb{E}\Big[\sum_{a \in A}\sum_{b \in B}\sqrt{\text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b))}\Big] - \text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b)) \\
&= \sum_{a \in A}\sum_{b \in B}\Big(\mathbb{E}\Big[\sqrt{\text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b))}\Big]\Big) - \text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b))
\\
&= \sum_{a \in A}\sum_{b \in B}\Big(\mathbb{E}\Big[\frac{w_a^Tw_b}{||w_a||_2||w_b||_2}\Big]\Big) - \text{tr}(\mathcal{P}(w_a)\mathcal{P}(w_b))
\end{aligned}
$$
Note that the bias here is expressed with respect to the original WEAT score component function, which summarizes the cosine matrix with the arithmetic mean in one direction and the arithmetic *sum* in the other direction rather than taking the arithmetic grand mean. Although the arithmetic grand mean is used more frequently in subsequent applications, it is more informative to consider the bias in the estimator due to the linear dependence internal to $A$ and $B$, and we can temporarily overlook the bias due to the extra $1/k$ scaling factor in the MCS estimator. Conversely, if the estimator were only off by a multiplicative factor, it would be simple enough to just multiply it by this factor. Unfortunately, the bias is more complex than this.

As previously noted, when $k = 1$ we can simply square the MCS estimator to correct the bias.
