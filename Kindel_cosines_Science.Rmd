...
Minimality generalizes to many dimensions in the sense that it applies recursively to every $[1,k-1]$-dimensional comparison contained in any $k$-dimensional comparison. This can be explained geometrically. Consider the $k$-simplex formed by the pairwise Euclidean differences between a set of vectors indexed by any keyword list $X$. (Cosine similarity is the central angle subtended by this vector.) The $k$-simplex is bounded by the $q$-simplical subspaces formed by subsets of $X$ ($q \in [1, k-1], q \in \mathbb{Z}$). An admissible similarity metric must satisfy the minimality criterion with respect to all of the $q$-simplices of equal dimensionality, or it is geometrically inconsistent with itself. As a concrete example, consider the tetrahedron formed by $X=\{\text{he, him, his, himself}\}$. $X$ has triangular faces $X_A = \{\text{he, him, himself}\}$ and $X_B = \{\text{he, him, his}\}$. $X_A$ and $X_B$ coincide at the edge between $\{\text{he, him}\}$, so the dihedral angle between $A$ and $B$ is proportional to the pointwise distance between $\{\text{his}\}$ and $\{\text{himself}\}$. Now consider a proposed metric $\text{Sim}^*$ that yields the result $\text{Sim}^*(X_A, X_A) < \text{Sim}^*(X_A, X_B)$. This implies that the distance between $\{\text{himself}\}$ and $\{\text{his}\}$ must be less than zero, but this is not possible, so $\text{Sim}^*$ is inadmissible.

Define a multidimensional similarity metric as *minimally internally consistent* if every $q$-subset of any $k$-subset of the rows of a real vector space $W$ is more similar to itself than any other $q$-subset of the same $k$-subset. Formally, where $[W]^{kCq}$ denotes the set of $q$-subsets of some $k$-subset of $W$ $(k > q)$, then the following statement must be true:  
$$\forall i, \forall q < k: \arg\sup_{j} \text{Sim}([W]^{kCq}_i, [W]^{kCq}_j) = i$$  
In other words, if I choose any set of keywords, then every subset I can make by removing one or more words from the set must be more similar to itself with respect to my chosen similarity metric than any of the other subsets with the same number of words. Define the index of inconsistency $\mathcal{I}_{q,k}(X)$ for a similarity metric with respect to $X \subset W$ as the discrete distribution of the number of maximally self-similar subsets belonging to all $q$-subsets of a list of cardinality $k$. A minimally internally consistent metric places all of its mass on $q$ for any choice of $q$ and $k$. It is particularly worrisome if the expected value of $\mathcal{I}_{q,k}(X)$ is decreasing as $k$ increases; adding more relevant keywords to an analysis should not make it less consistent. The index of inconsistency is closely related to the irregularity of the polytope induced by $X$. A helpful heuristic discrepancy measure is the Euclidean condition number of $X$, which measures the elongation of its hyperelliptical projection [@golub2013matrix, p.88].


**A canonical metric for multidimensional similarity.** To construct a minimally internally consistent multidimensional similarity metric, define $W(k)$ as the space of subspaces spanned by $k$-subsets of the rows of $W$, and for all subspaces $A \in W(k)$ consider the orthogonal projection operator onto $A$, $\mathcal{P}(A) = A(A^TA)^{-1}A^T$. Then, equip $W(k)$ with the quadratic form $\text{Sim}_\text{CCA; k}: W(k) \times W(k) \rightarrow \mathbb{R}$ corresponding to the Frobenius inner product of the orthogonal projections [@krzanowski1979between; @meyer2000matrix, p. 429--430; @borg2005modern, p. 439--441]:
